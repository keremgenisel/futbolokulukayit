...
describe("WhatsApp saf mantık", () => {
  it("waNumara: 05xx, +90, boşluklu biçimler 905… olur; geçersiz boş", () => {
    ...
    expect(waNumara("")).toBe(""); expect(waNumara(null)).toBe("");
  });
  it("sablonDoldur: yer tutucular dolar, bilinmeyen boşalır, boşluklar sadeleşir, satır sonu korunur", () => {
    expect(sablonDoldur("Sayın {veli}, {oyuncu} için {ay} aidatı ({kalan}).\n{kulup}", { veli: "Ayşe Yıldız", oyuncu: "Kaan", ay: "Eylül 2026", kalan: "3.500 ₺", kulup: "Eyüpspor" }))
  ...
  it("aidatDegerleri ve varsayılan aidat şablonu birlikte anlamlı mesaj üretir", () => {
    const d = aidatDegerleri({ veli_ad: "Ayşe Yıldız", ad_soyad: "Kaan Yıldız", yil: 2026, ay: 9, tutar: 3500, kalan: 2500, odeme_donemi: "1-10", yas_grubu_ad: "U11" });
    expect(d).toMatchObject({ veli: "Ayşe Yıldız", ay: "Eylül 2026", tutar: "3.500 ₺", kalan: "2.500 ₺", donem: "1-10", grup: "U11", kulup: "Eyüpspor Futbol Okulu" });
    const m = sablonDoldur(VARSAYILAN_SABLONLAR.aidat, d);
    expect(m).toContain("Kaan Yıldız için Eylül 2026 aidatı (2.500 ₺)"); expect(m).toContain("her ayın 1-10 günleridir"); expect(m.endsWith("Eyüpspor Futbol Okulu")).toBe(true);
  });
  it("antrenmanDegerleri: değişiklik notundan eski tarih/saat, iptal nedeni 'İptal' ise boş", () => {
    ...
    expect(d).toMatchObject({ eskiTarih: "7 Eylül 2026 Pazartesi", eskiSaat: "17:00", yeniTarih: "8 Eylül 2026 Salı", yeniSaat: "18:30", saha: "Saha 2", grup: "U11" });
    expect(sablonDoldur(VARSAYILAN_SABLONLAR.degisiklik, d)).toBe("Sayın Ayşe, U11 grubunun 7 Eylül 2026 Pazartesi 17:00 antrenmanı 8 Eylül 2026 Salı 18:30 saatine alınmıştır (Saha 2).\nEyüpspor Futbol Okulu");
    const ip = antrenmanDegerleri({ tarih: "2026-09-08", saat: "18:30", yas_grubu_ad: "U11", iptal_nedeni: "İptal" }, { ad_soyad: "Kaan" });
    expect(sablonDoldur(VARSAYILAN_SABLONLAR.iptal, ip)).toBe("Sayın Veli, U11 grubunun 8 Eylül 2026 Salı 18:30 antrenmanı iptal edilmiştir.\nEyüpspor Futbol Okulu");
    expect(antrenmanDegerleri({ tarih: "2026-09-08", degisiklik_notu: "bozuk{", yas_grubu_ad: "U9" }, { ad_soyad: "A" }).eskiTarih).toBe("8 Eylül 2026 Salı");
  });
