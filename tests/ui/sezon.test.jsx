...
const gruplar = [{ id: 1, ad: "U11", aktif: 1 }, { id: 2, ad: "U12", aktif: 1 }];
const adaylar = [
...
describe("Ayarlar > Yeni Sezon sihirbazı", () => {
  it("yenileyenler işaretlenir, üst grup önerilir, geçiş tek çağrıyla yapılır", async () => {
    ...
    window.okul = { db: vi.fn(async (fn, a) => {
      ...
      if (fn === "yeniSezonaGec") { durum = { ...durum, aktifSezon: a.sezon, sonGecis: "2027-09-01T10:00:00Z" }; return { ok: true, sezon: a.sezon, yenilenen: a.yenileyenler.length, pasif: 3 - a.yenileyenler.length, grupDegisen: 1, borcSilinen: a.eskiBorcSil ? 2 : 0 }; }
    ...
    await waitFor(() => expect(window.okul.db).toHaveBeenCalledWith("yeniSezonaGec", { sezon: "2027-2028", eskiBorcSil: true, yenileyenler: [{ id: 10, yas_grubu_id: 2 }, { id: 12, yas_grubu_id: 2 }] }));
  ...
  it("geçersiz sezon adıyla geçiş başlatılmaz; kullanıcı rolü bölümü göremez", async () => {
    ...
    cleanup();
    render(<ToastSaglayici><Ayarlar oturum={{ username: "hoca", role: "kullanici" }} saltOkunur={false} baslangicBolum="sezon" /></ToastSaglayici>);
...
describe("Pano: sezon sonu hatırlatması", () => {
  ...
  it("aktif sezon eskiyse uyarı ve Yeni Sezona Geç düğmesi; güncelse uyarı yok", async () => {
    const onSezon = vi.fn();
    kur("2000-2001");
    render(<ToastSaglayici><Pano onOyuncu={() => {}} onSekme={() => {}} onMakbuzKes={() => {}} onSezon={onSezon} /></ToastSaglayici>);
