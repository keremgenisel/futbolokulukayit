...
const alici = (k, ek = {}) => ({ key: k, player_id: Number(k), guardian_id: 100 + Number(k), oyuncu_ad: `Oyuncu ${k}`, veli_ad: `Veli ${k}`, grup: "U11", numara: "0532 111 22 3" + k, onay: 1, mesaj_id: null, hatirlatma: 0, ek: { kalan: "3.500 ₺", gecikme: 12 }, degerler: aidatDegerleri({ veli_ad: `Veli ${k}`, ad_soyad: `Oyuncu ${k}`, yil: 2026, ay: 9, tutar: 3500, kalan: 3500, odeme_donemi: "1-10", yas_grubu_ad: "U11" }), ...ek });
...
describe("WhatsApp toplu hatırlatma penceresi", () => {
  ...
  beforeEach(() => {
    ...
    window.okul = {
      ...
      db: vi.fn(async (fn, ...a) => {
        ...
        if (fn === "mesajKaydet") { const id = kayitlar.length + 1; kayitlar.push({ id, ...a[0] }); return { id }; }
        if (fn === "mesajSil") { kayitlar = kayitlar.filter((m) => m.id !== a[0]); return {}; }
        return null;
      }),
    };
  });
  const kur = (alicilar, ek = {}) => render(<ToastSaglayici><WhatsAppHatirlat tur="aidat" baslik="Borçlulara WhatsApp ile Hatırlat" altBaslik="Eylül 2026" alicilar={alicilar} kayit={{ yil: 2026, ay: 9 }} saltOkunur={false} onKapat={vi.fn()} {...ek} /></ToastSaglayici>);
  ...
  it("uygun veliye WhatsApp'ta Aç: wa numarası + doldurulmuş şablonla köprü çağrılır, kayıt yazılır, satır Hatırlatıldı olur; Geri al siler", async () => {
    ...
    await waitFor(() => expect(window.okul.app.whatsappAc).toHaveBeenCalledWith("905321112231", expect.stringContaining("Sayın Veli 1, Oyuncu 1 için Eylül 2026 aidatı (3.500 ₺)")));
  ...
  it("düzenlenebilir modda (genel mesaj) metin değiştirilip o metinle açılır; bu ay ikinci hatırlatma uyarısı", async () => {
    render(<ToastSaglayici><WhatsAppHatirlat tur="genel" baslik="WhatsApp Mesajı" alicilar={[alici("1", { hatirlatma: 1 })]} duzenlenebilir saltOkunur={false} onKapat={vi.fn()} /></ToastSaglayici>);
...
describe("Pano — WhatsApp hatırlatma girişleri", () => {
  beforeEach(() => {
    window.okul = { app: { whatsappAc: vi.fn(async () => ({ ok: true })) }, db: vi.fn(async (fn, ...a) => {
      ...
      if (fn === "listUnpaid") return [
        { id: 5, player_id: 2, ad_soyad: "Kaan Yıldız", yas_grubu_ad: "U11", odeme_donemi: "1-10", yil: 2026, ay: 9, tutar: 3500, veli_tel: "05321112233", veli_ad: "Ayşe Yıldız", veli_id: 7, veli_wa: "05321112233", veli_onay: 1, hatirlatma: 1, son_hatirlatma: "2026-09-05 10:00:00" },
        { id: 6, player_id: 3, ad_soyad: "Cem Polat", yas_grubu_ad: "U12", odeme_donemi: "1-10", yil: 2026, ay: 9, tutar: 3500, veli_tel: "05329998877", veli_ad: "Ali Polat", veli_id: 8, veli_wa: "05329998877", veli_onay: 0, hatirlatma: 0 },
      ];
  ...
  it("satır düğmesi onaysız velide kapalı; 'Borçlulara Hatırlat' toplu pencereyi açar, hatırlatılan tarih satırda görünür", async () => {
    ...
    await waitFor(() => expect(window.okul.app.whatsappAc).toHaveBeenCalledWith("905321112233", expect.stringContaining("Kaan Yıldız için Eylül 2026 aidatı (3.500 ₺)")));
    await waitFor(() => expect(window.okul.db).toHaveBeenCalledWith("mesajKaydet", expect.objectContaining({ player_id: 2, guardian_id: 7, tur: "aidat", yil: 2026, ay: 9 })));
  });
});
...
describe("Ayarlar > WhatsApp Mesajları", () => {
  ...
  it("şablon düzenlenince önizleme örnek oyuncuyla güncellenir, tek Kaydet ayara yazar, 'Varsayılan metne dön' geri alır", async () => {
    render(<ToastSaglayici><Ayarlar oturum={{ username: "admin", role: "admin" }} saltOkunur={false} baslangicBolum="whatsapp" /></ToastSaglayici>);
    ...
    await waitFor(() => expect(window.okul.db).toHaveBeenCalledWith("setSetting", "wa_sablon_aidat", "Merhaba {veli}, {oyuncu} {ay} aidatı: {kalan}"));
    ...
    expect(screen.getByTestId("wa-ayar-onizleme")).toHaveTextContent("7 Eylül 2026 Pazartesi 17:00 antrenmanı 8 Eylül 2026 Salı 18:30 saatine alınmıştır (Saha 2)");
  });
});
