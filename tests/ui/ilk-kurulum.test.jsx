...
describe("İlk kurulum sihirbazı", () => {
  it("kulüp adı → aidat/indirim → gruplar+sezon → yedek (atla) → kurtarma kodları → bitir: her adım doğru çağrıyı yapar", async () => {
    ...
    window.okul = {
      db: vi.fn(async (fn, ...a) => {
        ...
        if (fn === "createAgeGroup") {
          gruplar.push(a[0].ad);
          return { id: gruplar.length, ad: a[0].ad };
        }
    ...
    render(
      <ToastSaglayici>
        <IlkKurulum oturum={{ username: "admin", ad_soyad: "Yönetici", role: "admin" }} onBitti={onBitti} />
      </ToastSaglayici>,
    );
    // 1. Kulüp
    ...
    await waitFor(() =>
      expect(window.okul.db).toHaveBeenCalledWith("aidatAyarlariKaydet", {
        kalemler: [{ id: 1, varsayilan_fiyat: 3500 }],
        indirimler: { burslu: 100, indirimli: 0, kardes: 15 },
      }),
    );
    // 3. Gruplar + sezon
  ...
  it("'Şimdi değil' kurulumu tamam işaretlemez, sonraki girişte yeniden çıkar", async () => {
    ...
    window.okul = {
      db: vi.fn(async (fn, ...a) => {
        ...
        return fn === "listAgeGroups" ? [] : null;
      }),
      yedek: { durum: vi.fn(async () => ({})) },
      auth: {},
    };
    const onBitti = vi.fn();
    render(
      <ToastSaglayici>
        <IlkKurulum oturum={{ username: "admin", role: "admin" }} onBitti={onBitti} />
      </ToastSaglayici>,
    );
