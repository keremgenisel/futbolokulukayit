...
function Deneme() { const toast = useToast(); return <button type="button" onClick={() => toast("ok", "Kaydedildi")}>Göster</button>; }

describe("Bildirimler (toast)", () => {
  it("en üstte ortada çıkar (sağ altta değil) ve modal'ların üstünde kalır", () => {
    render(<ToastSaglayici><Deneme /></ToastSaglayici>);
    ...
    expect(k.style.position).toBe("fixed"); expect(k.style.top).toBe("20px"); expect(k.style.left).toBe("50%"); expect(k.style.transform).toContain("translateX(-50%)");
    expect(k.style.bottom).toBe(""); expect(k.style.right).toBe("");
