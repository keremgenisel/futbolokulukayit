...
describe("Excel oyuncu aktarımı — satır çözümleme", () => {
  ...
  it("satırlar çözülür: grup eşleşir/yeni grup, durum/ücret/dönem, veli, TC kontrolü, mükerrer atlama", () => {
    ...
    expect(r.kayitlar[1]).toMatchObject({
      yas_grubu_id: 2,
      durum: "deneme",
      ucret_tipi: "burslu",
      aylik_aidat: 0,
  ...
  it("Ayarlar'dan eklenen ücret tipi adı ya da koduyla tanınır; tanınmayan Normal olur ve uyarı verir", () => {
    const tipler = [
      { kod: "sampiyon_bursu", ad: "Şampiyon Bursu" },
      { kod: "kardes", ad: "Kardeş İndirimi" },
    ];
    const r = satirlariCoz(
      [
        ["Ad Soyad", "Doğum Tarihi", "Ücret Tipi"],
        ["A", "01.01.2015", "Şampiyon Bursu"],
        ["B", "01.01.2015", "sampiyon_bursu"],
        ["C", "01.01.2015", "Bilinmeyen"],
      ],
      { gruplar: [], ucretTipleri: tipler },
    );
  ...
  it("Mesaj Onayı sütunu: boş/evet → onaylı, hayır/0 → onaysız", () => {
    const r = satirlariCoz(
      [
        ["Ad Soyad", "Doğum Tarihi", "Veli Adı", "Veli Telefonu", "Mesaj Onayı"],
        ["A", "01.01.2015", "Veli A", "05321234567", ""],
        ["B", "01.01.2015", "Veli B", "05321234568", "Hayır"],
        ["C", "01.01.2015", "Veli C", "05321234569", "evet"],
        ["D", "01.01.2015", "Veli D", "05321234560", "0"],
      ],
      { gruplar: [] },
    );
