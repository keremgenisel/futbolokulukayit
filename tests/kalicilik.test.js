...
const kos = (dizin, adim, env = {}) =>
  spawnSync(electronBin, [path.join(root, "scripts", "tests", "kalicilik.cjs"), dizin, adim], {
    encoding: "utf-8",
    timeout: 120000,
    env: { ...process.env, ...env },
  });
...
describe("Veri kalıcılığı (Electron altında, dist/ gerekli)", () => {
  const senaryolar = [
    ["düzgün kapanış (pencere kapatma)", {}],
    ["kaba kapanış (SIGKILL)", { KABA_KAPANIS: "1" }],
  ];
  for (const [ad, env] of senaryolar) {
    it(
      ad + " sonrası parola, grup, oyuncu, ayar, makbuz, yoklama ve lisans meta yerinde",
      () => {
        ...
        const o = kos(dizin, "oku", env); // senaryo bilgisi oku adımına da geçer (SIGKILL sonrası bilgi satırları)
