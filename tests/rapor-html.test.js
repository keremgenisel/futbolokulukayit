...
const sutunlar = [{ baslik: "Ad Soyad", anahtar: "ad" }, { baslik: "Tutar", anahtar: "tutar", sag: true }];
const satirlar = [{ ad: "Kaan Yıldız", tutar: "3.500 ₺" }, { ad: "<b>Ali</b> & Veli", tutar: "150 ₺" }];
...
describe("raporHtml", () => {
  it("başlık, alt başlık, sütun başlıkları, satırlar ve kayıt sayısı yer alır; sağa hizalı sütun işaretlenir", () => {
    ...
    expect(h).toContain("<th>Ad Soyad</th>"); expect(h).toContain('<th class="sag">Tutar</th>');
    expect(h).toContain("<td>Kaan Yıldız</td>"); expect(h).toContain('<td class="sag">3.500 ₺</td>');
  ...
  it("hücre içeriği kaçırılır (HTML enjeksiyonu yok)", () => {
    ...
    expect(h).toContain("&lt;b&gt;Ali&lt;/b&gt; &amp; Veli"); expect(h).not.toContain("<b>Ali</b>");
  });
  it("yatay sayfa, alt başlık yoksa alt satırı yok, boş liste 0 kayıt", () => {
    const h = raporHtml({ baslik: "x", sutunlar, satirlar: [], yatay: true });
    expect(h).toContain("size: A4 landscape"); expect(h).not.toContain('class="alt"'); expect(h).toContain("0 kayıt");
  });
  it("logo yalnız güvenli data URL ise girer", () => {
    expect(raporHtml({ baslik: "x", sutunlar, satirlar, logo: "data:image/png;base64,iVBORw0KGgo=" })).toContain('<img src="data:image/png;base64,iVBORw0KGgo=" alt="">');
