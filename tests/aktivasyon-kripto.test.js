...
describe("Aktivasyon kripto sözleşmesi — uygulama (Node) ↔ sunucu (Web Crypto)", () => {
  ...
  it("Sunucu (Web Crypto) imzalı LEASE uygulamada (Node) doğrulanır", async () => {
    ...
    const lease = await leaseImzala({ firma: "Test A.Ş.", makineId: "MAK-1", leaseBitis: "2026-08-01", iptal: false, uretimTarihi: "2026-07-19" }, await ozelAnahtarYukle(privPem));
  ...
  it("tahrif edilen anahtar sunucuda reddedilir (bitiş uzatma)", async () => {
    ...
    const k = JSON.parse(Buffer.from(veri, "base64url").toString()); k.bitis = "2099-01-01";
  ...
  it("yabancı anahtarla imzalı lease uygulamada reddedilir (çift eşleşmiyor)", async () => {
    ...
    const lease = await leaseImzala({ firma: "X", makineId: "MAK-1", leaseBitis: "2026-08-01", iptal: false, uretimTarihi: "2026-07-19" }, await ozelAnahtarYukle(yabanci));
