    ...
    prepare(sql) {
      ...
      const api = {
        ...
        first: async () => {
          ...
          if (sql.includes("COUNT(*) AS n FROM kurulumlar"))
            return { n: t.kurulumlar.filter((x) => x.lisansId === args[0] && x.aktif === 1).length };
          if (sql.includes("FROM kurulumlar WHERE lisansId = ? AND makineId"))
            return t.kurulumlar.find((x) => x.lisansId === args[0] && x.makineId === args[1]) || null;
          return null;
        },
        run: async () => {
          if (sql.includes("INSERT INTO lisanslar")) {
            ...
            if (r) Object.assign(r, { sonGoruldu, surum, aktif: 1 });
          }
          return { success: true };
        },
        all: async () => {
          if (sql.includes("FROM lisanslar l")) {
            return {
              results: t.lisanslar.map((l) => ({
                firma: l.firma,
                bitis: l.bitis,
                maksKullanici: l.maksKullanici,
                maksKurulum: l.maksKurulum,
                iptal: l.iptal,
...
beforeAll(() => {
  ...
  anahtar = imzala(
    { firma: "T A.Ş.", bitis: "2027-01-01", maksKullanici: 5, uretimTarihi: "2026-07-19", aktivasyonGerekli: true },
    privPem,
  );
...
describe("Aktivasyon sunucusu — kurulum limiti + iptal + admin", () => {
  ...
  it("üçüncü makine kurulum limitine takılır (403)", async () => {
    ...
    expect(c.body.error).toMatch(/limit/);
  });
  ...
  it("bilinmeyen makine yenilemede 403 (önce aktive edilmeli)", async () => {
    ...
    expect(r.status).toBe(403);
    expect(r.body.error).toMatch(/kurulum bulunamadı/);
  });
  ...
  it("uzaktan iptal: iptal sonrası yenileme 403 (lease penceresi dolunca uygulama kilitlenir)", async () => {
    ...
    const r = await call("/yenile", "POST", { anahtar, makineId: "MAK-A" });
    expect(r.status).toBe(403);
    expect(r.body.error).toMatch(/iptal/);
  });
  ...
  it("admin token sabit zamanlı karşılaştırma; aktivasyon hız sınırı (inceleme #25)", async () => {
    ...
    expect(tokenEsit("abc", "abc")).toBe(true);
    expect(tokenEsit("abc", "abd")).toBe(false);
    expect(tokenEsit("abc", "abcd")).toBe(false);
    expect(tokenEsit(null, "")).toBe(true);
    const t0 = 1_000_000;
    let asildi = false;
