  ...
  const tx = db.islem(() => {
    ...
    for (const k of kayitlar) {
      ...
      if (yeni_grup && !p.yas_grubu_id) {
        if (!grupId.has(yeni_grup)) {
          const g = db.createAgeGroup({ ad: yeni_grup, sezon: db.getSetting("aktif_sezon") || "" });
          grupId.set(yeni_grup, g.id);
          yeniGrup++;
        }
      ...
      if (veli)
        db.addGuardian(o.id, {
          ...
          gsm: veli.gsm || "",
          whatsapp_no: veli.gsm || "",
          veli_mi: 1,
          mesaj_onayi: veli.mesaj_onayi === undefined ? 1 : veli.mesaj_onayi,
        });
      eklenen++;
    }
...
function registerAktarHandlers(getSession) {
  ...
  ipcMain.handle("aktar:sablon", async (e) => {
    ...
    const r = await dialog.showSaveDialog(BrowserWindow.fromWebContents(e.sender), {
      defaultPath: "oyuncu-sablonu.xlsx",
  ...
  ipcMain.handle("aktar:onizle", async (e) => {
    ...
    try {
      ...
      const sonuc = satirlariCoz(satirlar, {
        gruplar: db.listAgeGroups(),
        ucretTipleri: db.listFeeTypes(),
        mevcutTc: new Set(oyuncular.map((o) => o.tc_no).filter(Boolean)),
        mevcutPasaport: new Set(oyuncular.map((o) => o.pasaport_no).filter(Boolean)),
      });
      return { ok: true, dosya: r.filePaths[0], ...sonuc };
    } catch (err) {
      return { error: "Excel okunamadı: " + err.message };
    }
  });
  ipcMain.handle("aktar:uygula", async (_e, kayitlar) => {
    ...
    try {
      return aktarUygula(kayitlar);
    } catch (err) {
      return { error: "Aktarım başarısız, hiçbir kayıt yazılmadı: " + err.message };
    }
  });
}
