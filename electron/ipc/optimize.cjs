...
function dosyalariTopla(kok) {
  ...
  const gez = (d) => { for (const ad of fs.readdirSync(d)) { const t = path.join(d, ad); const st = fs.statSync(t); if (st.isDirectory()) gez(t); else out.push({ yol: t, boyut: st.size, ad }); } };
  gez(kok);
  return out;
}
...
function analiz(kok = db.getUploadsDir()) {
  ...
  const gruplar = {};
  for (const d of resimler) { const k = kategori(d, kok); (gruplar[k] ||= { adet: 0, bayt: 0 }); gruplar[k].adet++; gruplar[k].bayt += d.boyut; }
  return {
    ok: true,
...
function uygula(kok = db.getUploadsDir()) {
  ...
  let once = 0, sonra = 0, kucultulen = 0;
  for (const d of resimler) {
    ...
    if (yeni.length < buf.length) {
      ...
      fs.writeFileSync(gecici, yeni); fs.renameSync(gecici, d.yol); // atomik değişim
      kucultulen++;
    }
