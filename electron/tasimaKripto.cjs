...
// Dosya: MAGIC(9) | salt(16) | iv(12) | tag(16) | şifreli veri.  Uzantı: .fokpaket
// Aynı kap, farklı MAGIC ile normal yedek için de kullanılır (inceleme #6): parola = makine anahtarı, uzantı .fokyedek.
const crypto = require("crypto");
const MAGIC = Buffer.from("FOKTASI1.", "ascii");
const YEDEK_MAGIC = Buffer.from("FOKYEDK1.", "ascii");
...
function coz(paket, parola, { magic = MAGIC } = {}) {
  if (!paketMi(paket, magic))
    throw new Error(
      magic.equals(YEDEK_MAGIC)
        ? "Bu bir Futbol Okulu Kayıt Programı yedeği değil"
