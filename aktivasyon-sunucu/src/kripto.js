...
export const acikAnahtarYukle = (pem) =>
  crypto.subtle.importKey("spki", pemDer(pem), { name: "Ed25519" }, false, ["verify"]);
export const ozelAnahtarYukle = (pem) =>
  crypto.subtle.importKey("pkcs8", pemDer(pem), { name: "Ed25519" }, false, ["sign"]);
...
export async function lisansDogrula(anahtar, acikAnahtar) {
  const p = String(anahtar || "").trim().split(".");
  ...
  try { return { gecerli: true, payload: JSON.parse(dec.decode(veri)) }; }
