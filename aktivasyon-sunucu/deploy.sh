#!/bin/sh
# Eyüpspor aktivasyon sunucusunu Cloudflare'a ilk kez kurar. Bir kez `npx wrangler login` gerekir.
...
if grep -q "DEPLOY-SONRASI-DOLDUR" wrangler.toml; then
  ...
  ID=$(npx wrangler d1 create eyupspor-lisans 2>&1 | grep -o 'database_id = "[^"]*"' | cut -d'"' -f2)
...
npx wrangler deploy
echo
echo "Deploy tamam. Çıktıdaki https://eyupspor-aktivasyon.<hesap>.workers.dev adresini"
