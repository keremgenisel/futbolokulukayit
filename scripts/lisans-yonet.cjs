...
if (!SUNUCU) {
  console.error("HATA: AKTIVASYON_URL boş (electron/aktivasyonIstemci.cjs). Önce sunucuyu deploy edip adresi göm.");
  process.exit(1);
}
...
const post = async (yol, govde) => {
  const r = await fetch(SUNUCU + yol, {
    method: "POST",
...
const mevcutKurulum = async (anahtar) =>
...
(async () => {
  ...
  try {
    if (komut === "tumu") {
      ...
      if (!list.length) {
        console.log("Kayıtlı lisans yok.");
        return;
      }
      console.log(`${list.length} lisans:\n`);
      console.log(
        "FİRMA".padEnd(28) + "BİTİŞ".padEnd(13) + "KULLANICI".padEnd(11) + "KURULUM".padEnd(11) + "DURUM".padEnd(9) + "OLUŞTURULDU",
      );
      ...
      console.log(
        r.body.ok
          ? "✅ İptal edildi. Yenileme kesildi; mevcut lease penceresi (LEASE_GUN) dolunca uygulama salt-okunura düşer."
          : `❌ ${r.body.error || "hata " + r.status}`,
      );
    } else if (komut === "ac") {
    ...
    console.error("Sunucuya ulaşılamadı:", e.message);
    process.exit(1);
  }
})();
