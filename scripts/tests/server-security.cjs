...
app.whenReady().then(async () => {
  try {
    ...
    check(
      "yanlış parola 401",
      (await istek("/api/auth/login", { method: "POST", body: { username: "admin", password: "yanlis" } })).status === 401,
    );
    ...
    check(
      "zorunlu parola değişimi öncesi /api/db 403",
      (await istek("/api/db", { method: "POST", body: { fn: "listAgeGroups", args: [] }, token: giris.body.token })).status === 403,
    );
    const ilkDeg = await istek("/api/auth/changePassword", {
      method: "POST",
      body: { newPassword: "admin-parola-1" },
    ...
    check(
      "jetonla okuma",
      (await istek("/api/db", { method: "POST", body: { fn: "listFeeItems", args: [] }, token: tok })).body.sonuc.length >= 10,
    );
    ...
    check(
      "beyaz liste dışı fonksiyon 403",
      (await istek("/api/db", { method: "POST", body: { fn: "close", args: [] }, token: tok })).status === 403,
    );

    // Yönetici olmayan kullanıcı → admin işlemi 403
    await istek("/api/db", {
      method: "POST",
      body: { fn: "createUser", args: [{ username: "antrenor", password: "antrenor1", ad_soyad: "Antrenör", role: "kullanici" }] },
      token: tok,
    });
    ...
    check(
      "yönetici olmayan createUser 403",
      (
        await istek("/api/db", {
          method: "POST",
          body: { fn: "createUser", args: [{ username: "x", password: "xxxxxx" }] },
          token: g2.body.token,
        })
      ).status === 403,
    );
    ...
    check(
      ...
      (await istek("/api/db", { method: "POST", body: { fn: "listAgeGroups", args: [] }, token: g2.body.token })).status === 200,
    );

    // Parola değişimi: eski jeton düşer, yeni jeton çalışır
    check(
      "eski parolasız değişim 400 (inceleme #14)",
      (await istek("/api/auth/changePassword", { method: "POST", body: { newPassword: "yeni-parola-1" }, token: g2.body.token })).status ===
        400,
    );
    check(
      "kısa yeni parola 400",
      (
        await istek("/api/auth/changePassword", {
          method: "POST",
          body: { newPassword: "kisa7", oldPassword: "antrenor1" },
          token: g2.body.token,
        })
      ).status === 400,
    );
    const pd = await istek("/api/auth/changePassword", {
      method: "POST",
      body: { newPassword: "yeni-parola-1", oldPassword: "antrenor1" },
      token: g2.body.token,
    });
    ...
    // Dosya yükleme
    const oyuncu = await istek("/api/db", {
      method: "POST",
      body: { fn: "createPlayer", args: [{ ad_soyad: "Test", dogum_tarihi: "2015-01-01", yas_grubu_id: grp.body.sonuc.id }] },
      token: tok,
    });
    const up = await istek("/api/files/addDocument", {
      method: "POST",
      body: { playerId: oyuncu.body.sonuc.id, tip: "saglik", ad: "rapor.pdf", base64: Buffer.from("%PDF-1.4 test").toString("base64") },
      token: tok,
    });
    check("belge yükleme", up.status === 200 && fs.existsSync(path.join(db.getUploadsDir(), up.body.dosya_yolu)));
    const foto = (ad) =>
      istek("/api/files/addDocument", {
        method: "POST",
        body: { playerId: oyuncu.body.sonuc.id, tip: "foto", ad, base64: Buffer.from("PNG test").toString("base64") },
        token: tok,
      });
    const f1 = await foto("v1.png");
    const f2 = await foto("v2.png");
    const fotoKayitlari = db.listDocuments(oyuncu.body.sonuc.id).filter((b) => b.tip === "foto");
    check(
      "vesikalık sunucuda da tek dosya: eski dosya silinir",
      f2.status === 200 &&
        fotoKayitlari.length === 1 &&
        fotoKayitlari[0].dosya_yolu === f2.body.dosya_yolu &&
        !fs.existsSync(path.join(db.getUploadsDir(), f1.body.dosya_yolu)),
    );
    check(
      "yasak uzantı reddedilir",
      (
        await istek("/api/files/addDocument", {
          method: "POST",
          body: { playerId: oyuncu.body.sonuc.id, tip: "diger", ad: "zararli.exe", base64: "AA==" },
          token: tok,
        })
      ).status === 400,
    );
    ...
    check(
      "kurtarma üretimi oturumsuz 401",
      (await istek("/api/auth/kurtarmaUret", { method: "POST", body: { userId: 1 } })).status === 401,
    );
    const kullanici = await istek("/api/db", {
      method: "POST",
      body: { fn: "createUser", args: [{ username: "veli", password: "veli-parola-1", role: "kullanici" }] },
      token: tok,
    });
    ...
    check(
      "eski kodla sıfırlama 401",
      (
        await istek("/api/auth/kurtarmaSifirla", {
          method: "POST",
          body: { username: "veli", kod: kendi.body.kodlar[0], yeniParola: "yeni-parola-1" },
        })
      ).status === 401,
    );
    check(
      ...
      (
        await istek("/api/auth/kurtarmaSifirla", {
          method: "POST",
          body: { username: "veli", kod: yonetici.body.kodlar[0], yeniParola: "123" },
        })
      ).status === 400,
    );
    const sfr = await istek("/api/auth/kurtarmaSifirla", {
      method: "POST",
      body: { username: "veli", kod: yonetici.body.kodlar[0], yeniParola: "yeni-parola-1" },
    });
    check(
      "doğru kod parolayı sıfırlar, eski jeton düşer",
      sfr.status === 200 &&
        sfr.body.kalan === 7 &&
        (await istek("/api/auth/me", { token: veliTok })).status === 401 &&
    ...
    for (let i = 0; i < 6; i++)
      sonKod = (
        await istek("/api/auth/kurtarmaSifirla", {
          method: "POST",
          body: { username: "veli", kod: "ZZZZ-ZZZZ", yeniParola: "yeni-parola-2" },
        })
      ).status;
    ...
    check(
      "kendi hesabını silme 400",
      (await istek("/api/db", { method: "POST", body: { fn: "deleteUser", args: [1] }, token: tok })).status === 400,
    );
    check(
      "kullanıcı rolü silemez 403",
      (
        await istek("/api/db", {
          method: "POST",
          body: { fn: "deleteUser", args: [kullanici.body.sonuc.id] },
          token: (await istek("/api/auth/login", { method: "POST", body: { username: "veli", password: "yeni-parola-1" } })).body.token,
        })
      ).status === 403,
    );
    await istek("/api/db", {
      method: "POST",
      body: { fn: "createUser", args: [{ username: "yonetici2", password: "test-sifre-y2", role: "admin" }] },
      token: tok,
    });
    const y2Tok = (await istek("/api/auth/login", { method: "POST", body: { username: "yonetici2", password: "test-sifre-y2" } })).body
      .token;
    const silme = await istek("/api/db", { method: "POST", body: { fn: "deleteUser", args: [1] }, token: y2Tok });
    check(
      "yeni yönetici ilk admin'i siler, admin jetonu düşer",
      silme.status === 200 && silme.body.sonuc.ok === true && (await istek("/api/auth/me", { token: tok })).status === 401,
    );

    // Login brute-force: 8 yanlış → 429
    let son = 0;
    for (let i = 0; i < 10; i++)
      son = (await istek("/api/auth/login", { method: "POST", body: { username: "admin", password: "kotu" + i } })).status;
    check("login hız sınırı 429", son === 429);

    // Sabitleme: farklı sertifikayla bağlantı reddedilir
    const yabanci = crypto.generateKeyPairSync("ed25519").publicKey.export({ type: "spki", format: "pem" });
    void yabanci;
    let red = false;
    try {
      await pinliFetch(base + "/saglik", {
        dispatcher: pinliDispatcher(
          require("selfsigned").generate
            ? (await require("selfsigned").generate([{ name: "commonName", value: "x" }], { keySize: 2048, days: 1 })).cert
            : pem,
        ),
      });
    } catch {
