...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const { registerGuncellemeHandlers } = require("../../electron/ipc/guncelleme.cjs");
    ...
    registerGuncellemeHandlers({ getSession });
    ...
    db.createReceipt({
      ...
      satirlar: [{ fee_item_id: aidat.id, tutar: 3500, aciklama: "Aidat", yil: t.getFullYear(), ay: t.getMonth() + 1 }],
    });
    ...
    const shot = async (ad) => {
      ...
      fs.writeFileSync(path.join(cikti, ad + ".png"), img.toPNG());
      console.log("shot", ad);
    };
    const js = (kod) => win.webContents.executeJavaScript(kod, true);
    const tikla = async (metin, sec = "button") => {
      const ok = await js(
        `(() => { const b = [...document.querySelectorAll("${sec}")].find(x => x.textContent.trim().startsWith(${JSON.stringify(metin)})); if (!b) return false; b.click(); return true; })()`,
      );
    ...
    await js(
      `(() => { const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value").set; const ins = [...document.querySelectorAll("input[type=password]")]; for (const i of ins) { set.call(i, "yeniparola1"); i.dispatchEvent(new Event("input", { bubbles: true })); } })()`,
    );
    await tikla("Kaydet");
    await bekle(600);
    await shot("03-pano");
    ...
    await js(`document.querySelector("button[aria-label='Menüyü genişlet']").click()`);
    await bekle(400);
    await js(
      `(() => { const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value").set; const i = document.querySelector("input[aria-label='Tesise giriş araması']"); set.call(i, "Ka"); i.dispatchEvent(new Event("input", { bubbles: true })); })()`,
    );
    await bekle(600);
    await shot("04-pano-arama");
    await tikla("Oyuncular");
    await bekle(500);
    await shot("05-oyuncular");
    // Boş formda Kaydet: zorunlu alan uyarısı formun üstünde görünmeli
    await tikla("Yeni Oyuncu");
    await bekle(400);
    await tikla("Oyuncuyu Kaydet");
    await bekle(300);
    await shot("05b-yeni-oyuncu-uyari");
    ...
    await bekle(300);
    await js(`[...document.querySelectorAll("tbody tr")].find((tr) => tr.textContent.includes("Kaan Yıldız")).click()`);
    await bekle(600);
    await shot("06-oyuncu-karti");
    await tikla("Belgeler");
    await bekle(300);
    await shot("07-belgeler");
    await tikla("Ödemeler");
    await bekle(300);
    await shot("08-odemeler");
    await tikla("Makbuz Kes");
    await bekle(800);
    await shot("09-tahsilat");
    await tikla("Kaydet");
    await bekle(1500);
    await shot("10-tahsilat-sonrasi");
    await tikla("Yoklama");
    await bekle(600);
    await js(
      `(() => { const b = [...document.querySelectorAll("button")].find(x => x.textContent.startsWith("U12")); b && b.click(); })()`,
    );
    await bekle(600);
    await shot("11-yoklama");
    // WhatsApp (plan §13): Yoklama'da Düzenle formu; Pano'da Borçlulara Hatırlat penceresi; Ayarlar > WhatsApp Mesajları
    if (await js(`!![...document.querySelectorAll("button")].find((b) => b.textContent.trim() === "Düzenle")`)) {
      await tikla("Düzenle");
      await bekle(300);
      await shot("11b-antrenman-duzenle");
      await tikla("Vazgeç");
    }
    await tikla("Pano");
    await bekle(700);
    if (await js(`!![...document.querySelectorAll("button")].find((b) => b.textContent.trim() === "Borçlulara Hatırlat")`)) {
      await tikla("Borçlulara Hatırlat");
      await bekle(700);
      await shot("03c-whatsapp-hatirlat");
      await js(`document.querySelector("[role=dialog] button[aria-label='Pencereyi kapat']")?.click()`);
      await bekle(300);
    }
    await tikla("Raporlar");
    await bekle(400);
    await tikla("Önizle");
    await bekle(600);
    ...
    await js(
      `(() => { const s = document.querySelector("select[aria-label='Ay']"); s.value = ""; s.dispatchEvent(new Event("change", { bubbles: true })); })()`,
    );
    ...
    await tikla("Kaydet ve Devam");
    await bekle(400);
    await shot("13a2-sihirbaz-yas-gruplari"); // kalabalık grup ipucu bu adımda
    await js(`[...document.querySelectorAll("button")].find((b) => b.textContent.trim() === "Şimdi değil")?.click()`);
    await bekle(400);
    await tikla("Ayarlar");
    await bekle(400);
    ...
    await bekle(500);
    await js(
      `(() => { const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value").set; for (const [sel, v] of [["input[aria-label='Forma fiyatı']", "9000"], ["input[aria-label='Mont fiyatı']", "8000"]]) { const i = document.querySelector(sel); set.call(i, v); i.dispatchEvent(new Event("input", { bubbles: true })); } })()`,
    );
    await bekle(300);
    await shot("13b-aidat-kalemleri");
    ...
    await bekle(200);
    await js(`[...document.querySelectorAll("h3")].find((h) => /Ücret Tipleri/.test(h.textContent))?.scrollIntoView({ block: "start" })`);
    await bekle(300);
    await shot("13c-ucret-tipleri");
    ...
    if (await js(`!!document.querySelector("[role=dialog]")`)) {
      ...
      await bekle(500);
    } // kaydedilmemiş kalem değişikliği uyarısı
    ...
    await bekle(500);
    await shot("13c-kullanicilar");
    await tikla("Lisans");
    await bekle(400);
    await shot("14-lisans");
    await tikla("Yeni Sezon");
    await bekle(600);
    await js(`document.querySelector("input[aria-label='Kerem Yılmaz yeniledi']")?.click()`);
    await bekle(300);
    await shot("14b-yeni-sezon");
    ...
    await bekle(300);
    await shot("15c-tasima-paketi");
    await tikla("Hakkında");
    await bekle(400);
    await tikla("Güncelleme Denetle");
    await bekle(500);
    await shot("16-hakkinda-guncelleme"); // dev modda "denetlenmez" bilgisi
    await tikla("Resim ve Belge Optimizasyonu");
    await bekle(300);
    await tikla("Analiz Et");
    await bekle(600);
    await shot("15b-optimizasyon");
    ...
    console.error("SMOKE HATA:", e && e.stack);
    kod = 1;
  }
  try {
