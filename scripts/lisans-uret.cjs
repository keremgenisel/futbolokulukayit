...
if (!fs.existsSync(privateYol)) {
  console.error(`Özel anahtar bulunamadı: ${privateYol}\nİlk kez üretmek için: node scripts/lisans-anahtar-cifti.cjs`);
  process.exit(1);
}
...
if (!kontrol.gecerli) {
  console.error("HATA: üretilen anahtar gömülü açık anahtarla doğrulanamadı (özel anahtar uygulamadakiyle eşleşmiyor).");
  process.exit(1);
}
