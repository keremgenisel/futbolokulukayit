...
async function getir() {
  ...
  if (!bekleyen) {
    bekleyen = (async () => {
      try {
        const l = await window.okul?.db("listFeeTypes");
        if (Array.isArray(l) && l.length) onbellek = l;
      } catch {
        /* varsayılan kalır */
      }
      bekleyen = null;
...
export function ucretTipleriYenile() {
  onbellek = null;
  getir().then(() => dinleyenler.forEach((f) => f(onbellek)));
}
