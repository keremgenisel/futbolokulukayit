...
export function TakvimSeridi({ secili, bugun, baslangic, onSec, onBaslangic, gunOzetleri }) {
  ...
  const klavye = (e) => {
    ...
    if (e.key === "ArrowLeft") { e.preventDefault(); onSec(gunKaydir(secili, -1)); }
    else if (e.key === "ArrowRight") { e.preventDefault(); onSec(gunKaydir(secili, 1)); }
    else if (e.key === "Home") { e.preventDefault(); onSec(bugun); onBaslangic(varsayilanBaslangic(bugun)); }
  };
  const bugune = () => { onSec(bugun); onBaslangic(varsayilanBaslangic(bugun)); };
  const okStil = { width: 40, alignSelf: "stretch", borderRadius: 10, border: "1px solid var(--cizgi)", background: "#fff", color: "var(--mor)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 };

  return (
    <div ref={kok} tabIndex={0} onKeyDown={klavye} aria-label="Gün şeridi" style={{ background: "#fff", border: "1px solid var(--cizgi)", borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 10, outline: "none" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <button type="button" aria-label="Önceki hafta" title="Önceki hafta" onClick={() => onBaslangic(gunKaydir(baslangic, -7))} style={okStil}><Ikon ad="sol" boyut={20} /></button>
        {hucreler.map((h) => {
          ...
          return (
            <button key={h.iso} type="button" onClick={() => onSec(h.iso)} aria-label={etiket} aria-pressed={on} data-iso={h.iso} style={{
              position: "relative", flex: "1 1 0", minWidth: 0, height: 84, borderRadius: 10, cursor: "pointer",
              background: on ? "var(--mor)" : "#fff", border: `1px solid ${on ? "var(--mor)" : "var(--cizgi)"}`,
              ...
              <span style={{ fontSize: 10, lineHeight: "12px", height: 12, fontWeight: 700, letterSpacing: ".08em", color: on ? "var(--sari)" : "var(--mor)" }}>{h.ayEtiketi ? h.ay : ""}</span>
              <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", color: on ? "#D8CCE9" : "var(--soluk)" }}>{h.gunAdi}</span>
              <span className="baslik" style={{ fontSize: 26, fontWeight: 700, lineHeight: 1 }}>{h.gun}</span>
              <span style={{ display: "flex", gap: 3, height: 7, alignItems: "center" }}>
                {noktalar.map((n, i) => <span key={i} title={NOKTA_AD[n]} data-nokta={n} style={{ width: 7, height: 7, borderRadius: "50%", background: (on ? NOKTA_RENK_SECILI : NOKTA_RENK)[n] }} />)}
              </span>
              {h.bugunMu && <span style={{ position: "absolute", left: 14, right: 14, bottom: 6, height: 3, borderRadius: 2, background: "var(--sari)" }} />}
            </button>
          );
        })}
        <button type="button" aria-label="Sonraki hafta" title="Sonraki hafta" onClick={() => onBaslangic(gunKaydir(baslangic, 7))} style={okStil}><Ikon ad="sag" boyut={20} /></button>
      </div>
