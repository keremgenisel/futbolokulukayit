...
export function WhatsAppHatirlat({ baslik, altBaslik, tur, alicilar, kayit = {}, grup, saltOkunur, onKapat, onDegisti, duzenlenebilir }) {
  ...
  useEffect(() => { sablonOku(tur).then(setAyar).catch((e) => toast("err", hataMetni(e))); }, [tur]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const metin = (a) => (duzenlenebilir && metinDuzen !== null ? metinDuzen : sablonDoldur(ayar?.sablon || "", { ...a.degerler, kulup: ayar?.kulup }));
  ...
  const ac = async (a) => {
    ...
    try {
      ...
      if (!saltOkunur) {
        const k = await db("mesajKaydet", { player_id: a.player_id, guardian_id: a.guardian_id || null, tur, yil: kayit.yil ?? null, ay: kayit.ay ?? null, training_id: kayit.training_id ?? null, metin: m });
  ...
  return (
    <Modal onKapat={onKapat} genislik={1120} yukseklik="min(760px, 92vh)"
      ust={<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", background: "var(--mor)", color: "#fff", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap" }}><span className="baslik" style={{ color: "#fff", fontSize: 26, fontWeight: 700 }}>{baslik}</span>{altBaslik && <span style={{ color: "#D8CCE9", fontSize: 14 }}>{altBaslik}</span>}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {uygunlar.length > 1 && <span data-testid="wa-sayac" style={{ background: "var(--sari)", color: "var(--mor-koyu)", fontWeight: 700, fontSize: 14, padding: "6px 12px", borderRadius: 999 }}>{yapilan} / {uygunlar.length} {etiket}</span>}
          <button type="button" onClick={onKapat} aria-label="Pencereyi kapat" style={{ background: "none", border: 0, color: "#D8CCE9", fontSize: 22, cursor: "pointer" }}>×</button>
        </div>
      </div>}
      altBar={<>
        <span style={{ flex: 1, alignSelf: "center", color: "var(--soluk)", fontSize: 13 }}>{engelli > 0 ? `${engelli} veliye açılamaz: ${satirlar.filter((s) => !s.uygun.ok).map((s) => s.uygun.neden).reduce((m, n) => ({ ...m, [n]: (m[n] || 0) + 1 }), {}) && Object.entries(satirlar.filter((s) => !s.uygun.ok).reduce((m, s) => ({ ...m, [s.uygun.neden]: (m[s.uygun.neden] || 0) + 1 }), {})).map(([n, c]) => `${c} ${n.toLocaleLowerCase("tr-TR")}`).join(", ")}` : ""}</span>
      ...
      <div style={{ display: "flex", gap: 20, minHeight: 0, flex: 1 }}>
        <div style={{ flex: 1, minWidth: 0, overflow: "auto" }}>
          <table><thead><tr><th>Veli · Oyuncu</th><th>Numara</th>{tur === "aidat" && <th>Kalan</th>}{tur === "aidat" && <th>Gecikme</th>}<th></th></tr></thead><tbody>
            {satirlar.map((s) => (
              <tr key={s.key} onClick={() => setSecili(s.key)} style={{ cursor: "pointer", opacity: s.uygun.ok ? 1 : .6, background: secili === s.key ? "var(--mor-acik)" : undefined }}>
                <td><div style={{ fontWeight: 600 }}>{s.veli_ad || "Veli yok"}</div><div style={{ fontSize: 12.5, color: "var(--soluk)" }}>{s.oyuncu_ad}{s.grup ? ` · ${s.grup}` : ""}</div>{!s.uygun.ok && <div style={{ fontSize: 12, color: "var(--kirmizi)" }}>{s.uygun.neden}</div>}{s.uygun.ok && s.hatirlatma > 0 && !s.mesaj_id && <div style={{ fontSize: 12, color: "var(--kirmizi)" }}>bu ay {s.hatirlatma + 1}. kez</div>}</td>
                <td>{s.numara || "—"}</td>
                {tur === "aidat" && <td style={{ fontWeight: 700 }}>{s.ek?.kalan}</td>}
                {tur === "aidat" && <td style={{ color: s.ek?.gecikme > 0 ? "var(--kirmizi)" : "var(--soluk)" }}>{s.ek?.gecikme > 0 ? `${s.ek.gecikme} gün` : "—"}</td>}
                <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                  {s.mesaj_id ? <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><Rozet ton="green">{Etiket}</Rozet>{s.mesaj_id > 0 && !saltOkunur && <button type="button" onClick={(e) => { e.stopPropagation(); geriAl(s); }} style={{ background: "none", border: 0, color: "var(--soluk)", cursor: "pointer", fontSize: 12.5, textDecoration: "underline" }}>Geri al</button>}</span>
                    : <Btn kucuk tur={s.uygun.ok ? "yesil" : "ghost"} ikon={<Ikon ad="whatsapp" boyut={16} />} disabled={!s.uygun.ok || bekliyor || !ayar} onClick={(e) => { e.stopPropagation(); ac(s); }} aria-label={`${s.veli_ad || s.oyuncu_ad} WhatsApp'ta aç`}>WhatsApp'ta Aç</Btn>}
                </td>
              </tr>
            ))}
          </tbody></table>
        </div>
        <div style={{ width: 340, flexShrink: 0, borderLeft: "1px solid var(--cizgi)", paddingLeft: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          <span style={{ fontSize: 12, color: "var(--soluk)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".06em" }}>Mesaj önizlemesi{seciliSatir ? ` · ${seciliSatir.veli_ad || seciliSatir.oyuncu_ad}` : ""}</span>
          {seciliSatir && ayar && (duzenlenebilir
            ? <textarea aria-label="Mesaj metni" value={metin(seciliSatir)} onChange={(e) => setMetinDuzen(e.target.value)} rows={9} style={{ width: "100%", boxSizing: "border-box", border: "1px solid var(--cizgi)", borderRadius: 10, padding: 12, fontSize: 14, lineHeight: 1.45, fontFamily: "inherit", resize: "vertical" }} />
            : <div data-testid="wa-onizleme" style={{ background: "#DCF8C6", borderRadius: "12px 12px 12px 2px", padding: "12px 14px", fontSize: 14, lineHeight: 1.45, whiteSpace: "pre-line" }}>{metin(seciliSatir)}</div>)}
