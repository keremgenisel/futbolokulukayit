...
export function Pano({ onOyuncu, onSekme, onMakbuzKes, saltOkunur, onSezon }) {
  ...
  const waAlici = (b) => ({ key: String(b.player_id), player_id: b.player_id, guardian_id: b.veli_id || null, oyuncu_ad: b.ad_soyad, veli_ad: b.veli_ad, grup: b.yas_grubu_ad, numara: b.veli_wa || b.veli_tel || "", onay: b.veli_onay, mesaj_id: null, hatirlatma: b.hatirlatma || 0, son_hatirlatma: b.son_hatirlatma, ek: { kalan: paraTR(b.kalan ?? b.tutar), gecikme: gecikmeGunu(b.odeme_donemi, b.yil, b.ay, new Date()) }, degerler: aidatDegerleri(b) });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Kart style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><h3 style={{ fontSize: 22 }}>{AY_ADLARI[ay - 1]} Aidatı Ödemeyenler</h3><div style={{ display: "flex", gap: 12, alignItems: "center" }}>{borclular.length > 0 && <Btn kucuk tur="yesil" ikon={<Ikon ad="whatsapp" boyut={16} />} onClick={() => setWa({ baslik: "Borçlulara WhatsApp ile Hatırlat", altBaslik: `${AY_ADLARI[ay - 1]} ${yil} · ${borclular.length} borçlu`, alicilar: borclular.map(waAlici) })}>Borçlulara Hatırlat</Btn>}<a href="#" onClick={(e) => { e.preventDefault(); onSekme("oyuncular", "borclu"); }} style={{ fontSize: 14, fontWeight: 600, textDecoration: "none" }}>Tümü ({borclular.length})</a></div></div>
          {borclular.length === 0 ? <div style={{ color: "var(--soluk)" }}>Borçlu oyuncu yok.</div> : (
            <table><thead><tr><th>Oyuncu</th><th>Grup</th><th>Veli telefonu</th><th>Ödeme dönemi</th><th>Gecikme</th><th></th></tr></thead><tbody>
              {borclular.slice(0, 8).map((b) => { const g = gecikmeGunu(b.odeme_donemi, b.yil, b.ay, new Date()); return <tr key={b.id} onClick={() => onOyuncu(b.player_id)} style={{ cursor: "pointer" }}><td style={{ fontWeight: 600 }}>{b.ad_soyad}</td><td>{b.yas_grubu_ad || "—"}</td><td><Telefon no={b.veli_tel} etiket={b.veli_ad} /></td><td>{b.odeme_donemi}</td><td style={{ color: g > 0 ? "var(--kirmizi)" : "var(--soluk)" }}>{g > 0 ? `${g} gün` : "—"}{b.durum === "kismi" && <span style={{ display: "block", fontSize: 12 }}>kalan {paraTR(b.kalan)}</span>}{b.hatirlatma > 0 && <span style={{ display: "block", fontSize: 12, color: "var(--soluk)" }}>hatırlatıldı {tarihTR(String(b.son_hatirlatma).slice(0, 10))}</span>}</td><td><div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>{(() => { const u = hatirlatmaUygunMu({ numara: b.veli_wa || b.veli_tel, onay: b.veli_onay }); return <Btn kucuk tur="ghost" ikon={<Ikon ad="whatsapp" boyut={16} />} disabled={!u.ok} title={u.ok ? "WhatsApp ile aidat hatırlat" : u.neden} aria-label={`${b.ad_soyad} WhatsApp`} style={{ color: u.ok ? "var(--yesil)" : undefined, padding: "0 8px" }} onClick={(e) => { e.stopPropagation(); setWa({ baslik: "WhatsApp ile Aidat Hatırlat", altBaslik: `${AY_ADLARI[ay - 1]} ${yil}`, alicilar: [waAlici(b)] }); }} />; })()}{!saltOkunur && <Btn kucuk tur="sari" onClick={(e) => { e.stopPropagation(); onMakbuzKes(b.player_id); }}>Makbuz</Btn>}</div></td></tr>; })}
            </tbody></table>
          )}
        </Kart>
      </div>
      {wa && <WhatsAppHatirlat tur="aidat" baslik={wa.baslik} altBaslik={wa.altBaslik} alicilar={wa.alicilar} kayit={{ yil, ay }} saltOkunur={saltOkunur} onKapat={() => { setWa(null); db("listUnpaid", yil, ay).then(setBorclular).catch(() => {}); }} />}
    </div>
  );
}
