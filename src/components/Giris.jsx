...
function sonKullaniciOku() {
  try {
    return localStorage.getItem(SON_KULLANICI) || "";
  } catch {
    return "";
  }
}
function sonKullaniciYaz(ad) {
  try {
    localStorage.setItem(SON_KULLANICI, ad);
  } catch {
    /* özel pencere vb. */
  }
}
...
export function Giris({ onGiris, mod, onModDegisti, marka }) {
  ...
  const gonder = async (e) => {
    ...
    setBekliyor(false);
    if (!r.ok) {
      setHata(r.error || "Giriş başarısız");
      return;
    }
    sonKullaniciYaz(ad);
    onGiris(r.user);
  ...
  const kurtarmaAc = () => {
    setKurtarma(true);
    setK({ kullanici: kullanici.trim(), kod: "", p1: "", p2: "" });
    setKMesaj({ tur: "", metin: "" });
  };
          ...
          <div style={{ width: 400, display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <Alan etiket="Kullanıcı adı">
              <input
                style={girisStili}
                value={k.kullanici}
                onChange={(e) => setK({ ...k, kullanici: e.target.value })}
                autoFocus={!k.kullanici}
              />
            </Alan>
            <Alan etiket="Kurtarma kodu">
              <input
                style={{ ...girisStili, fontFamily: "monospace", letterSpacing: ".08em" }}
                value={k.kod}
                onChange={(e) => setK({ ...k, kod: e.target.value.toUpperCase() })}
                placeholder="XXXX-XXXX"
                autoFocus={!!k.kullanici}
              />
            </Alan>
            <Alan etiket="Yeni parola">
              <input style={girisStili} type="password" value={k.p1} onChange={(e) => setK({ ...k, p1: e.target.value })} />
            </Alan>
            <Alan etiket="Yeni parola (tekrar)">
              <input style={girisStili} type="password" value={k.p2} onChange={(e) => setK({ ...k, p2: e.target.value })} />
            </Alan>
            {kMesaj.metin && (
              <div role="alert" style={{ color: kMesaj.tur === "err" ? "var(--kirmizi)" : "var(--yesil)", fontSize: 14, fontWeight: 600 }}>
                {kMesaj.metin}
              </div>
            )}
            <Btn type="submit" disabled={bekliyor} style={{ height: 48, justifyContent: "center", fontSize: 16 }}>
              Parolayı Sıfırla
            </Btn>
            <button
              type="button"
              onClick={() => setKurtarma(false)}
              style={{ background: "none", border: 0, color: "var(--mor)", cursor: "pointer", fontSize: 13, textDecoration: "underline" }}
            >
              Girişe dön
            </button>
            <span style={{ color: "var(--soluk)", fontSize: 13, textAlign: "center" }}>
              Kurtarma kodunuz yoksa bir yöneticiden parolanızı sıfırlamasını isteyin.
            </span>
          </div>
        </form>
      ) : (
          ...
          <div style={{ width: 400, display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Alan etiket="Kullanıcı adı">
              <div style={{ position: "relative" }}>
                ...
                <input
                  style={{ ...girisStili, paddingLeft: 40 }}
                  value={kullanici}
            ...
            {hata && (
              <div role="alert" style={{ color: "var(--kirmizi)", fontSize: 14, fontWeight: 600 }}>
                {hata}
              </div>
            )}
            {kMesaj.tur === "ok" && kMesaj.metin && (
              <div role="status" style={{ color: "var(--yesil)", fontSize: 14, fontWeight: 600 }}>
                {kMesaj.metin}
              </div>
            )}
            <Btn type="submit" disabled={bekliyor} style={{ height: 48, justifyContent: "center", fontSize: 16 }}>
            ...
            <span style={{ color: "var(--soluk)", fontSize: 13, textAlign: "center", display: "flex", flexDirection: "column", gap: 4 }}>
              {mod?.mode === "istemci" && (
                <span>
                  Sunucuya bağlı: <b>{mod.serverUrl}</b> · sunucudaki hesabınızla girin
                </span>
              )}
              <button
                type="button"
            ...
            {COKLU_PC_ACIK && mod && mod.mode !== "sunucu" && !baglanAcik && (
              <button
                type="button"
                onClick={() => setBaglanAcik(true)}
                style={{ background: "none", border: 0, color: "var(--mor)", cursor: "pointer", fontSize: 13, textDecoration: "underline" }}
              >
                {mod.mode === "istemci" ? "Sunucu adresini değiştir" : "Başka bilgisayardaki sunucuya bağlan"}
              </button>
            )}
            {COKLU_PC_ACIK && baglanAcik && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8, borderTop: "1px solid var(--cizgi)", paddingTop: 12 }}>
                <Alan etiket="Sunucu adresi">
                  <input style={girisStili} value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://100.x.x.x:3535" />
                </Alan>
                {fp && (
                  <div style={{ fontSize: 12, color: "var(--soluk)" }}>
                    Sunucu parmak izi: <b style={{ fontFamily: "monospace", wordBreak: "break-all" }}>{fp}</b> — sunucudaki Ayarlar &gt;
                    Sunucu ekranıyla aynıysa onaylayın.
                  </div>
                )}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <Btn
                    onClick={async () => {
                      ...
                      else if (r.needTrust || r.mismatch) {
                        setFp(r.fp);
                        if (r.mismatch) setBilgi("Sunucu sertifikası daha önce kaydedilenden farklı. Emin değilseniz onaylamayın.");
                      } else {
                        setBaglanAcik(false);
                        setFp(null);
